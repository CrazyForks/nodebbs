...
import { users, accounts, topics, posts, follows, bookmarks, categories, userRoles, roles, moderationLogs, sessions } from '../../db/schema.js';
import { eq, sql, desc, and, ne, like, inArray, count } from 'drizzle-orm';
import { userEnricher } from '../../services/userEnricher.js';
import { validateUsername, isAutoGeneratedUsername } from '../../utils/validateUsername.js';
...
export default async function userRoutes(fastify, options) {
  ...
  fastify.post('/:userId/restore', {
    ...
    // 检查是否在30天冷静期内
    const thirtyDaysAgo = new Date(Date.now() - DELETION_COOLDOWN_MS);
    if (new Date(user.deletionRequestedAt) < thirtyDaysAgo) {
      return reply.code(400).send({ error: '已超过30天冷静期，无法恢复' });
    }
